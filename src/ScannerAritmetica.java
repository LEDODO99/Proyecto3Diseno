	...
	public String ValidarAutomata(String cadena) {
		...
		for (int i = 0; i < cadena.length(); i++) {
			...
			for (int j = 0; j < nodoEvaluando.getTransiciones().size(); j++) {
				if (nodoEvaluando.getTransiciones().get(j).getParametro()
						.contains(Character.toString(cadena.charAt(i)))) {
			...
			if (!transitioned) {
				if (nodoEvaluando.getIsFinal()) {
					...
					tokenLocal = new Token(nodoEvaluando.getTokenNumber(), nodoEvaluando.getToken(),
